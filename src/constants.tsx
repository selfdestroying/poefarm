export const CATEGORIES: Categories = [
	{
		title: 'Currency',
		slug: 'Currency',
		icon: 'https://web.poecdn.com/image/Art/2DItems/Currency/CurrencyRerollRare.png?scale=1&w=1&h=1',
	},
	// 	{
	// 		title: 'Fragments',
	// 		slug: 'Fragment',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Maps/AtlasMaps/FragmentPhoenix.png?scale=1&w=1&h=1',
	// 	},
	// 	{
	// 		title: 'Coffins',
	// 		slug: 'Coffin',
	// 		icon: 'https://web.poecdn.com/gen/image/WzI1LDE0LHsiZiI6IjJESXRlbXMvQ3VycmVuY3kvTmVjcm9wb2xpcy9OZWNyb3BvbGlzQ29mZmluRmlsbGVkIiwidyI6MiwiaCI6MSwic2NhbGUiOjF9XQ/5b4214a420/NecropolisCoffinFilled.png',
	// 	},
	// 	{
	// 		title: 'Allflame Embers',
	// 		slug: 'AllflameEmber',
	// 		icon: 'https://web.poecdn.com/gen/image/WzI1LDE0LHsiZiI6IjJESXRlbXMvQ3VycmVuY3kvTmVjcm9wb2xpcy9BbGxmbGFtZUVtYmVyc0Jhc2VBem1lcmkiLCJ3IjoxLCJoIjoxLCJzY2FsZSI6MX1d/8960cc9f5b/AllflameEmbersBaseAzmeri.png',
	// 	},
	// 	{
	// 		title: 'Tattoos',
	// 		slug: 'Tattoo',
	// 		icon: 'https://web.poecdn.com/gen/image/WzI1LDE0LHsiZiI6IjJESXRlbXMvQ3VycmVuY3kvQW5jZXN0b3JzL0NvbW1vbkRleFRhdHR0b29FcXVpcG1lbnQiLCJ3IjoxLCJoIjoxLCJzY2FsZSI6MX1d/b80711f45f/CommonDexTatttooEquipment.png',
	// 	},
	// 	{
	// 		title: 'Omens',
	// 		slug: 'Omen',
	// 		icon: 'https://web.poecdn.com/gen/image/WzI1LDE0LHsiZiI6IjJESXRlbXMvQ3VycmVuY3kvQW5jZXN0b3JzL0FuY2VzdG9yT21lbjNCbHVlIiwidyI6MSwiaCI6MSwic2NhbGUiOjF9XQ/90021e045b/AncestorOmen3Blue.png',
	// 	},
	// 	{
	// 		title: 'Divination Cards',
	// 		slug: 'DivinationCard',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Divination/InventoryIcon.png?scale=1&w=1&h=1',
	// 	},
	// 	{
	// 		title: 'Artifacts',
	// 		slug: 'Artifact',
	// 		icon: 'https://web.poecdn.com/gen/image/WzI1LDE0LHsiZiI6IjJESXRlbXMvQ3VycmVuY3kvRXhwZWRpdGlvbi9HYW1ibGVyUmVmcmVzaEN1cnJlbmN5IiwidyI6MSwiaCI6MSwic2NhbGUiOjF9XQ/f878f8fc1a/GamblerRefreshCurrency.png',
	// 	},
	// 	{
	// 		title: 'Oils',
	// 		slug: 'Oil',
	// 		icon: 'https://web.poecdn.com/gen/image/WzI1LDE0LHsiZiI6IjJESXRlbXMvQ3VycmVuY3kvT2lscy9WYWFsT2lsIiwidyI6MSwiaCI6MSwic2NhbGUiOjF9XQ/54c3400bd8/VaalOil.png',
	// 	},
	// 	{
	// 		title: 'Incubators',
	// 		slug: 'Incubator',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Currency/Incubation/IncubationAbyss.png?scale=1&w=1&h=1',
	// 	},
	// 	{
	// 		title: 'Unique Weapons',
	// 		slug: 'UniqueReward',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Weapons/OneHandWeapons/OneHandSwords/Varunastra.png?scale=1&w=2&h=3',
	// 	},
	// 	{
	// 		title: 'Unique Armours',
	// 		slug: 'UniqueArmour',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Armours/Boots/Skyforth.png?scale=1&w=2&h=2',
	// 	},
	// 	{
	// 		title: 'Unique Accessories',
	// 		slug: 'UniqueAccessory',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Amulets/AgateAmuletUnique.png?scale=1&w=1&h=1',
	// 	},
	// 	{
	// 		title: 'Unique Flasks',
	// 		slug: 'UniqueFlask',
	// 		icon: 'https://web.poecdn.com/gen/image/WzksMTQseyJmIjoiMkRJdGVtc1wvRmxhc2tzXC9UYXN0ZU9mSGF0ZSIsInciOjEsImgiOjIsInNjYWxlIjp0cnVlLCJsZXZlbCI6MX1d/4727ad7a3a/Item.png',
	// 	},
	// 	{
	// 		title: 'Unique Jewels',
	// 		slug: 'UniqueJewel',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Jewels/unique7.png?scale=1&w=1&h=1',
	// 	},
	// 	{
	// 		title: 'Unique Relics',
	// 		slug: 'UniqueRelic',
	// 		icon: 'https://web.poecdn.com/gen/image/WzI1LDE0LHsiZiI6IjJESXRlbXMvUmVsaWNzL1JlbGljVW5pcXVlMngyIiwidyI6MiwiaCI6Miwic2NhbGUiOjF9XQ/15bd9eec94/RelicUnique2x2.png',
	// 	},
	// 	{
	// 		title: 'Skill Gems',
	// 		slug: 'SkillGem',
	// 		icon: 'https://web.poecdn.com/gen/image/WzI1LDE0LHsiZiI6IjJESXRlbXMvR2Vtcy9Qb3J0YWwiLCJ3IjoxLCJoIjoxLCJzY2FsZSI6MX1d/179ddc4cbd/Portal.png',
	// 	},
	// 	{
	// 		title: 'Cluster Jewels',
	// 		slug: 'ClusterJewel',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Jewels/NewGemBase2.png?w=1&h=1&scale=1',
	// 	},

	// 	{
	// 		title: 'Maps',
	// 		slug: 'Map',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Maps/AtlasMaps/Gorge3.png?scale=1&w=1&h=1',
	// 	},
	// 	{
	// 		title: 'Blighted Maps',
	// 		slug: 'BlightedMap',
	// 		icon: 'https://web.poecdn.com/gen/image/WzI4LDE0LHsiZiI6IjJESXRlbXNcL01hcHNcL0F0bGFzMk1hcHNcL05ld1wvTHVuYXJpc1RlbXBsZSIsInciOjEsImgiOjEsInNjYWxlIjp0cnVlLCJtbiI6MTAsIm10IjoxNCwibWIiOnRydWV9XQ/038c28d2e1/Item.png',
	// 	},
	// 	{
	// 		title: 'Blight-ravaged Maps',
	// 		slug: 'BlightRavagedMap',
	// 		icon: 'https://web.poecdn.com/gen/image/WzI4LDE0LHsiZiI6IjJESXRlbXMvTWFwcy9BdGxhczJNYXBzL05ldy9GaWVsZHMiLCJ3IjoxLCJoIjoxLCJzY2FsZSI6MSwibW4iOjEyLCJtdCI6MTYsIm11YiI6dHJ1ZX1d/a7ab3884c4/Fields.png',
	// 	},
	// 	{
	// 		title: 'Scourged Maps',
	// 		slug: 'ScourgedMap',
	// 		icon: 'https://web.poecdn.com/gen/image/WzI4LDE0LHsiZiI6IjJESXRlbXMvTWFwcy9BdGxhczJNYXBzL05ldy9Ucm9waWNhbElzbGFuZCIsInciOjEsImgiOjEsInNjYWxlIjoxLCJtbiI6MTIsIm10IjozLCJzY291cmdlZCI6MTB9XQ/9fce14dd94/TropicalIsland.png',
	// 	},
	// 	{
	// 		title: 'Unique Maps',
	// 		slug: 'UniqueMap',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Maps/UndeadSiege.png?scale=1&w=1&h=1',
	// 	},
	// 	{
	// 		title: 'Delirium Orbs',
	// 		slug: 'DeliriumOrb',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Currency/Delirium/DeliriumOrbScarabs.png?w=1&h=1&scale=1',
	// 	},
	// 	{
	// 		title: 'Invitations',
	// 		slug: 'Invitation',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Currency/Atlas/NullVoid5.png?w=1&h=1&scale=1',
	// 	},
	{
		title: 'Scarabs',
		slug: 'Scarab',
		icon: 'https://web.poecdn.com/image/Art/2DItems/Currency/Scarabs/GreaterScarabBreach.png?scale=1&scaleIndex=0&w=1&h=1',
	},
	// 	{
	// 		title: 'Memories',
	// 		slug: 'Memory',
	// 		icon: 'https://web.poecdn.com/gen/image/WzI1LDE0LHsiZiI6IjJESXRlbXMvQ3VycmVuY3kvTWVtb3J5TGluZS9OaWtvTWVtb3J5SXRlbSIsInciOjEsImgiOjEsInNjYWxlIjoxfV0/5c560ea8fd/NikoMemoryItem.png',
	// 	},
	// 	{
	// 		title: 'Base Types',
	// 		slug: 'BaseType',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Rings/OpalRing.png?scale=1&scaleIndex=0&w=1&h=1',
	// 	},
	{
		title: 'Fossils',
		slug: 'Fossil',
		icon: 'https://web.poecdn.com/image/Art/2DItems/Currency/Delve/SanctifiedFossil.png?w=1&h=1',
	},
	// 	{
	// 		title: 'Resonators',
	// 		slug: 'Resonator',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Currency/Delve/Reroll2x2A.png?w=1&h=1',
	// 	},
	// 	{
	// 		title: 'Beasts',
	// 		slug: 'Beast',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Currency/BestiaryOrbFull.png?scale=1&w=1&h=1',
	// 	},
	{
		title: 'Essences',
		slug: 'Essence',
		icon: 'https://web.poecdn.com/image/Art/2DItems/Currency/Essence/Woe7.png?scale=1&w=1&h=1',
	},
	// 	{
	// 		title: 'Vials',
	// 		slug: 'Vial',
	// 		icon: 'https://web.poecdn.com/image/Art/2DItems/Currency/VialTemperedFlesh.png?scale=1&w=1&h=1',
	// 	},
]
